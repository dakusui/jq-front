...
When it is searching for a file during internal inheritance resolution, it first searches for local node directory, which is created for temporarily, and if nothing is found, it will then traverses the variable.
...
----
          ...
          external inheritance             local node materialization
                 +----+                             +-----+
   +-------------+ {o}+-=----------+   +------------+ {o} +-=------------+
   |             +----+            |   |            +-----+              | 0...n
   V                               V   V                                 V
...
As shown, it consists of four main components, which are "external inheritance", "local node materialization", "internal inheritance", and "templating".
...
The 'local node directory' and its contents are utilized by the "internal inheritance' mechanism.
...
- [[[Cfront, 2]]] Cfront article in en.wikipedia.org. https://en.wikipedia.org/wiki/Cfront[Cfront]: 2019
- [[[cr, 3]]] Thayne McCombs. https://www.lucidchart.com/techblog/2018/07/16/why-json-isnt-a-good-configuration-language/[Why JSON isn’t a Good Configuration Language]: 2018.
- [[[yaml, 4]]] YAML article in en.wikipedia.org. https://en.wikipedia.org/wiki/YAML[YAML]: 2019
