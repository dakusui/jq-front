...
`powerjson` is a tool to cope with such challenges by introducing inheritance and templating into JSON files.

== Installation

Place the file `powerjson` somewhere on your `PATH`.

== Usage

[source, bash]
----
powerjson [-h|--help] [-e|--enable-templating] [-e|--enable-templating] TARGET
----
...
`$ref` is a built-in function of `powerjson`, which expands the value of the node specified by the path given as an argument.
Not only built-in functions but also any commands (bash expressions) valid on a platform on which `powerjson` is running can be used here.

For instance, following is a valid input to `powerjson`.
[source, json]
----
{
  "releaseVersion": "2.12.0",
...
And to enable it explicitly, you can use `-e` (`--enable-templating`) in case `POWERJSON_TEMPLATING_ENABLED` is set to `no`.

==== Built-in `powerjson` functions
...
'Private node directory' is inserted before the first entry of `POWERJSON_PATH` environment variable when a file to be expanded is searched during the step 1. b.
