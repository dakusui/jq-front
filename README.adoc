= jf: JSON with inheritance and templating

`jf`, which stands for "jq-front", is a simple tool to give your JSON files a power of inheritance and templating.
...
`jf` is a tool to cope with such challenges by introducing inheritance and templating into JSON files.

== Installation

=== Ubuntu
Place the file `jf` somewhere on your `PATH`.
Followings will be required by `jf`
...
```sh
jf [-h|--help] [-e|--enable-templating] [-d|--disable-templating] [--validation=no|strict|lenient] [TARGET]
```
...
With `jf`, you can build a JSON object reusing other JSON objects.
To specify JSON objects to be referenced, `jf` searches for keys which have special names in a given object.
...
Multiple inheritance is also supported by `jf`.
...
We can describe this relationship by using the templating feature of `jf`.
...
Once you render this file with `jf`, you will get the first file (Version file).

`$ref` is a built-in function of `jf`, which expands the value of the node specified by the path given as an argument.
Not only built-in functions but also any commands (bash expressions) valid on a platform on which `jf` is running can be used here.

For instance, following is a valid input to `jf`.
[source, json]
----
{
  "releaseVersion": "2.12.0",
...
You can control behaviours of `jf` by setting environment variables described in this section.

==== JF_PATH

From directories listed in `JF_PATH`, `jf` searches for requested file.
...
You can enable `jf` 's templating feature by setting this environment variable's value to `yes`.
...
Following is a diagram that illustrates `jf` 's processing pipeline design.
...
* Currently, almost no validation is performed by `jf`. (https://github.com/dakusui/jf/issues/23[Issue-23])
